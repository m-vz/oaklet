...
public:
    ...
    explicit FileLog(std::string logPath = CONFIG.DEFAULT_LOG_FILE_PATH, LogType logLevel = (const LogType) CONFIG.DEFAULT_FILE_LOG_LEVEL);
...
public:
    ...
    explicit Log(LogType logLevel = (const LogType) CONFIG.DEFAULT_LOG_LEVEL,
    ...
    friend Log& operator<<(Log &log, unsigned int message);
    friend Log& operator<<(Log &log, long int message);
    friend Log& operator<<(Log &log, unsigned long int message);
